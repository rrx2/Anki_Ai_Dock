...
class ConfigManager:
    ...
    def config_file(self):
        """Initializes config file path within the addon's own directory for portability."""
        if self._config_file is None:
            addon_dir = os.path.dirname(__file__)
            user_files_dir = os.path.join(addon_dir, "user_files")
            os.makedirs(user_files_dir, exist_ok=True)
            self._config_file = os.path.join(user_files_dir, "ai_dock_settings.json")
        return self._config_file
        
    @property  
    def backup_file(self):
        """Initializes backup file path within the addon's own directory."""
        if self._backup_file is None:
            addon_dir = os.path.dirname(__file__)
            user_files_dir = os.path.join(addon_dir, "user_files")
            os.makedirs(user_files_dir, exist_ok=True)
            self._backup_file = os.path.join(user_files_dir, "ai_dock_settings_backup.json")
